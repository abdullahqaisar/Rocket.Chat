...
export const dispatchInquiryQueued = async (inquiry: ILivechatInquiryRecord, agent?: SelectedAgent | null) => {
	console.log('🚀 ~ dispatchInquiryQueued ~ agent:', agent);
	const allowOfflineAgents = Boolean(settings.get('Livechat_allow_forward_inquiry_to_offline_department_agents'));
	console.log('🚀 ~ dispatchInquiryQueued ~ allowOfflineAgents:', allowOfflineAgents);
	...
	const { department, rid: roomId, v: visitor } = inquiry;
	...
	if (agent && allowOfflineAgents) {
		...
		await Promise.all([saveQueueInquiry(inquiry), sendMessageToAgent(livechatAgent, room, visitor)]);
		return;
	}
	...
	const sendMessageToOnlineAgent = onlineAgents.map(async (agent) => sendMessageToAgent(agent, room, visitor));
...
// const checkAgentBeforeTakeInquiry = async (agentId: string, roomId: string, departmentId: string, allowOfflineAgents = false) => {
// 	logger.debug(`Forwarding room ${roomId} to department ${departmentId} (to user ${agentId})`);
// 	const user = allowOfflineAgents ? await Users.findOneAgentById(agentId, {}) : await Users.findOneOnlineAgentById(agentId);
// 	console.log('🚀 ~ checkAgentBeforeTakeInquiry ~ user:', user);
// 	if (!user) {
...
const retrieveInquiry = async (roomId: string) => {
...
const setInquiryToDepartment = async (inquiry: ILivechatInquiryRecord, newDepartmentId?: string, agent?: SelectedAgent) => {
	console.log('🚀 ~ setInquiryToDepartment ~ inquiry:', inquiry);
	return RoutingManager.unassignAgent(inquiry, newDepartmentId, agent);
};

const setInquiryToAgent = async (inquiry: ILivechatInquiryRecord, agent: SelectedAgent) => {
	console.log('🚀 ~ setInquiryToDepartment ~ inquiry:', inquiry);
...
const checkIfThereAreOnlineAgentsInDepartment = async (departmentId: string) => {
...
export const forwardRoomToDepartment = async (room: IOmnichannelRoom, _guest: ILivechatVisitor, transferData: TransferData) => {
	if (!(await Omnichannel.isWithinMACLimit(room))) {
		throw new Error('maximum limit exceeded');
	}

	const { departmentId, oldDepartmentId } = checkDepartmentBeforeTakeInquiry(room._id, transferData.departmentId, room.departmentId);
	...
	if (allowOfflineDepartments) {
		return setInquiryToDepartment(inquiry, departmentId);
	}
	...
	return setInquiryToDepartment(inquiry, oldDepartmentId);
};
...
export const forwardRoomToAgent = async (room: IOmnichannelRoom, transferData: TransferData) => {
	...
	const agent = allowOfflineAgents
		? await Users.findOneAgentById(transferData.userId, {})
		: await Users.findOneOnlineAgentById(transferData.userId);
	console.log('🚀 ~ forwardRoomToAgent ~ agent:', agent);
	if (!agent) {
		logger.debug(`Agent ${transferData.userId} is offline. Cannot forward`);
		throw new Error('error-user-is-offline');
	}

	return setInquiryToAgent(inquiry, { ...agent, agentId: transferData.userId });
};
